...
pub struct SelectList<T: Ord + std::fmt::Debug> {
    common: BTreeSet<T>,
    selects: BTreeMap<String, BTreeSet<T>>,
}

impl<T: Ord + std::fmt::Debug> Default for SelectList<T> {
    fn default() -> Self {
        Self {
            common: BTreeSet::new(),
...
impl<T: Ord + std::fmt::Debug + Clone> SelectList<T> {
...
impl<T: Ord + std::fmt::Debug> Select<T> for SelectList<T> {
    fn configurations(&self) -> BTreeSet<Option<&String>> {
...
impl<T: Ord + std::fmt::Debug, U: Ord + std::fmt::Debug> SelectMap<T, U> for SelectList<T> {
